...
public class Algorithm8Direction extends JFrame implements ActionListener {
    // (10, 10) -> map rat nho
    ...
    public Algorithm8Direction(int[] size){
        row = size[0];
        column = size[1];
    ...
    // tinh gia tri heuristic su dung Diagonal Distance
    ...
    private void aStarAlgorithm() {
        ...
        while(!openList.isEmpty()) {
            ...
            if(node.point.equals(goal)) {
                JOptionPane.showMessageDialog(this, String.format("It's take %d steps to reach the goal", node.f));

                mainPanel.getComponent(getRealIndex(node.point.x, node.point.y)).setBackground(Color.RED);
                node = node.parent;
                while(node.parent != null) {
                    mainPanel.getComponent(getRealIndex(node.point.x, node.point.y)).setBackground(Color.YELLOW);
                    node = node.parent;
                }
                return;
            }
            ...
            for(int i = 0; i < 8; i++) {
                ...
                if(isValid(newPoint) && !isBlocked(newPoint)) {
                    ...
                    // neu khong thi them vao tap mo
                        ...
                        f[neighbour.point.x][neighbour.point.y] = neighbour.f;
                        openList.add(neighbour);
                        mainPanel.getComponent(getRealIndex(neighbour.point.x, neighbour.point.y)).setBackground(Color.CYAN);
                    }
                }
            }
        }
